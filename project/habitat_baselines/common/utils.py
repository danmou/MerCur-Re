...
class DiagGaussianNet(nn.Module):
    def __init__(self, num_inputs, num_outputs):
        ...
        self.log_std = nn.Parameter(torch.empty(num_outputs, 1), requires_grad=True)
        nn.init.constant_(self.log_std, 0)

    def forward(self, x):
        mean = self.mean(x)
        return CustomFixedNormal(mean, self.log_std.exp())
...
def generate_video(
    ...
    video_name = f"{scene_id}_episode{episode_id}_ckpt{checkpoint_idx}_{metric_name}{metric_value:.2f}"
