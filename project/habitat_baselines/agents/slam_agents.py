...
from project.habitat_baselines.config.default import get_config as cfg_baseline
from project.habitat_baselines.slambased.mappers import DirectDepthMapper
from project.habitat_baselines.slambased.path_planners import DifferentiableStarPlanner
from project.habitat_baselines.slambased.reprojection import (
...
from project.habitat_baselines.slambased.utils import generate_2dgrid

GOAL_SENSOR_UUID = "goal"
...
class ORBSLAM2Agent(RandomAgent):
    ...
    def update_internal_state(self, habitat_observation):
        ...
        try:
            ...
            print(f'Features: {self.slam.get_num_features()}, matched: {self.slam.get_num_matched_features()}')
            if self.timing:
        ...
        if self.tracking_is_OK:
            ...
            if len(self.position_history) > 1:
                ...
                self.stuck_on_obstacle = previous_step.item() <= 0.001  # hardcoded threshold for not moving
    ...
    def act(self, habitat_observation=None, random_prob=0.1):
        if habitat_observation is not None:
    ...
    def planner_prediction_to_command(self, p_next):
        ...
        d_angle = norm_ang(get_direction(p_init, p_next, ang_th=0.0, pos_th=0.0))
        print(f'Goal: {self.offset_to_goal[0].item():.1f}m, {abs(self.offset_to_goal[1]) * 180 / pi:.1f} degrees {"left" if self.offset_to_goal[1] > 0 else "right"}')
        print(f'Current pos: {p_init[[0, 2], 3].numpy()}, waypoint: {p_next[[0, 2], 3].numpy()}')
        print(f'Waypoint direction: {abs(d_angle) * 180 / pi:.1f} degrees {"left" if d_angle > 0 else "right"}')
        # action=1.0 => turn left pi/4, move 25 cm, turn again pi/4
        limit = 0.25
        command = min(limit, max(-limit, d_angle.item() * 4.0 / pi))
        print(f'Turning {abs(command) * 90:.1f} degrees {"left" if command > 0 else "right"}')
        return command
