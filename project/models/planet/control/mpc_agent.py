...
class MPCAgent(object):
  ...
  def __init__(self, batch_env, step, is_training, should_log, config, deterministic=False):
  ...
  def begin_episode(self, agent_indices):
    ...
    reset_state = nested.map(
        lambda var, val: tf.compat.v1.scatter_update(var, agent_indices, 0 * val),
  ...
  def perform(self, agent_indices, observ):
    observ['image'] = self._config.preprocess_fn(observ['image'])
    embedded = self._config.encoder({k: obs[:, None] for k, obs in observ.items()})[:, 0]
    state = nested.map(
        lambda tensor: tf.gather(tensor, agent_indices),
        self._state)
    ...
    remember_state = nested.map(
        lambda var, val: tf.compat.v1.scatter_update(var, agent_indices, val),
        self._state, state, flatten=True)
