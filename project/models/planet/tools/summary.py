...
from project.models.planet.tools import mask as masklib
  ...
  with tf.compat.v1.variable_scope(name):
    for name, dist in dists.items():
      ...
      if name in obs:
        ...
        summaries.append(tf.compat.v1.summary.scalar(name + '_log_prob', log_prob))
        abs_error = tf.reduce_mean(tf.abs(dist.mode() - obs[name]))
        summaries.append(tf.compat.v1.summary.scalar(name + '_abs_error', abs_error))
  return summaries
...
def objective_summaries(objectives, name='objectives'):
  ...
  with tf.compat.v1.variable_scope(name):
    for objective in objectives:
      summaries.append(tf.compat.v1.summary.scalar(objective.name, objective.value))
  return summaries
...
def prediction_summaries(dists, data, state, name='state'):
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    for key in ['action', 'taken_action']:
      if key in data:
        values = data[key][0]
        if values.shape.ndims == 1:
          values = values[:, None]
        values = tf.unstack(tf.transpose(values, (1, 0)))
        with tf.control_dependencies(summaries):
          summaries.append(plot_summary([key.title().replace('_', ' ')],
                                        [values],
                                        [list(range(len(values))) if len(values) > 1 else [None]],
                                        '{}_trajectory'.format(key)))
