...
class CustomOptimizer(object):
  ...
  def minimize(self, loss):
    with tf.name_scope('optimizer_{}'.format(self._name)):
      ...
      grad_norm = tf.compat.v1.global_norm(gradients)
      if self._clipping:
  ...
  def summarize(self, grad_norm):
    summaries = []
    with tf.name_scope('optimizer_{}'.format(self._name)):
      summaries.append(tf.compat.v1.summary.scalar('learning_rate', self._learning_rate))
      summaries.append(tf.compat.v1.summary.scalar('grad_norm', grad_norm))
      if self._clipping:
        clipped = tf.minimum(grad_norm, self._clipping)
        summaries.append(tf.compat.v1.summary.scalar('clipped_gradient_norm', clipped))
